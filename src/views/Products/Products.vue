<template>
  <main class="products view">
    <div class="view-header">
      <h1>Produtos</h1>

      <p>
        Este é o módulo de
        <span class="text-primary text-bold">Produtos</span>, onde você poderá
        fazer a gestão de todos os produtos armazenados pela ONG
      </p>
    </div>

    <div
      v-if="isLoading"
      class="view-content d-flex justify-content-center align-items-center"
    >
      <LoadingSpinner />
    </div>
    <div v-else>
      <Button category="primary" class="w-100" @click="openCreateProductModal">
        Cadastrar Produto
      </Button>

      <VueGoodTable
        class="mt-3 w-100"
        :rows="products"
        :columns="[
          {
            label: 'Nome',
            field: 'name',
          },
          {
            label: 'Descrição',
            field: 'description',
          },
          {
            label: 'Ações',
            field: 'actions',
          },
        ]"
        :pagination-options="paginationOptions"
      >
        <p slot="emptystate" class="text-center m-0">
          Nenhum produto encontrado
        </p>

        <template slot="table-row" slot-scope="props">
          <div
            v-if="props.column.field === 'actions'"
            class="media-body text-rigth d-flex justify-content-center"
          >
            <Button
              category="blue"
              class="mr-1"
              @click="openDetailsModal(props.row.id)"
            >
              <i class="fas fa-eye"></i>
            </Button>

            <Button
              category="warning"
              class="mr-1"
              @click="openEditModal(props.row.id)"
            >
              <i class="fas fa-edit"></i>
            </Button>

            <Button
              category="danger"
              @click="confirmProductDelete(props.row.id)"
            >
              <i class="fas fa-trash-alt ml-1"></i>
            </Button>
          </div>
        </template>
      </VueGoodTable>
    </div>

    <!-- Modal de cadastro e edição -->
    <Modal size="lg" :isVisible="modals.isCreateProductModalVisible">
      <template slot="modal-header">
        <h4 class="modal-title text-uppercase text-bold">
          {{ currentModalLabel }} produto
          <i class="fas fa-box-open"></i>
        </h4>
      </template>

      <template slot="modal-body">
        <form class="row" @submit="submitForm">
          <div class="form-group col-12">
            <label for="title">Nome *</label>
            <input
              id="name"
              type="text"
              v-model="form.name"
              class="form-control"
              placeholder="Digite o nome do produto"
              @focus="$v.form.name.$touch"
            />

            <div
              v-if="$v.form.name.$error"
              class="text-danger invalid-feedback"
              style="display: block"
            >
              <span v-if="!$v.form.name.required"
                >Nome do produto é obrigatório</span
              >
              <span v-if="!$v.form.name.minLength"
                >Nome do produto deve ter pelo menos 4 caracteres</span
              >
            </div>
          </div>

          <div class="form-group col-12">
            <label for="description">Descrição</label>
            <textarea
              style="min-height: 100px; max-height: 150px"
              id="description"
              type="text"
              v-model="form.description"
              class="form-control"
              placeholder="Digite a descrição do produto"
              @focus="$v.form.description.$touch"
            />

            <div
              v-if="$v.form.description.$error"
              class="text-danger invalid-feedback"
              style="display: block"
            >
              <span v-if="!$v.form.description.maxLength"
                >Descrição do produto não pode ter mais de 150 caracteres</span
              >
            </div>
          </div>
        </form>
      </template>

      <template slot="modal-footer">
        <Button category="danger" class="col-6" @click="closeCreateProductModal"
          >Cancelar <i class="fas fa-times-circle" />
        </Button>

        <Button
          category="primary"
          class="col-6 icon-rotate"
          @click="submitForm"
          :disabled="$v.form.$invalid"
        >
          Salvar
          <i class="fas fa-save ml-1"></i>
        </Button>
      </template>
    </Modal>

    <!-- Modal de detalhes -->
    <Modal size="lg" :isVisible="modals.isProductDetailsModalVisible">
      <template slot="modal-header">
        <h4 class="modal-title text-uppercase text-bold">
          Visualizar produto
          <i class="fas fa-box-open"></i>
        </h4>
      </template>

      <template slot="modal-body">
        <form class="row">
          <div class="form-group col-12">
            <label for="title">Nome *</label>
            <input
              id="name"
              type="text"
              v-model="form.name"
              class="form-control"
              placeholder="Digite o nome do produto"
              disabled
            />
          </div>

          <div class="form-group col-12">
            <label for="description">Descrição</label>
            <textarea
              style="min-height: 100px; max-height: 150px"
              id="description"
              type="text"
              v-model="form.description"
              class="form-control"
              placeholder="Digite a descrição do produto"
              disabled
            />
          </div>
        </form>
      </template>

      <template slot="modal-footer">
        <Button
          category="primary"
          class="col-12"
          @click="modals.isProductDetailsModalVisible = false"
          >Fechar <i class="fas fa-times-circle" />
        </Button>
      </template>
    </Modal>
  </main>
</template>

<script lang="ts">
/* eslint-disable @typescript-eslint/no-var-requires */
import 'vue-good-table/dist/vue-good-table.css';
import Vue from 'vue';
import { Button, Modal, LoadingSpinner } from '@/components';
import { mapActions, mapGetters } from 'vuex';
import { VueGoodTablePaginationOptions } from '@/utils/constants/VueGoodTablePaginationOptions';
import { maxLength, minLength, required } from 'vuelidate/lib/validators';
import { ActionResponse } from '@/store';
import showToast from '@/services/toast';
import alert from '@/services/alert';
import { productsRequests } from '@/services/api/requests';

export default Vue.extend({
  components: {
    Button,
    Modal,
    LoadingSpinner,
    VueGoodTable: require('vue-good-table').VueGoodTable,
  },

  mounted() {
    this.fetchProducts();
  },

  data: () => ({
    paginationOptions: VueGoodTablePaginationOptions,
    modals: {
      isProductDetailsModalVisible: false,
      isCreateProductModalVisible: false,
      currentModal: '' as 'create' | 'edit',
    },

    form: {
      id: '',
      name: '',
      description: '',
    },
  }),

  computed: {
    ...mapGetters({
      products: 'products/getProducts',
      isLoading: 'products/getIsLoading',
    }),

    currentModalLabel(): 'Cadastrar' | 'Editar' {
      const modalLabels = {
        create: 'Cadastrar',
        edit: 'Editar',
      } as const;

      return modalLabels[this.modals.currentModal];
    },
  },

  methods: {
    ...mapActions({
      fetchProducts: 'products/fetchProducts',
      addProduct: 'products/addProduct',
      deleteProduct: 'products/deleteProduct',
      updateProduct: 'products/updateProduct',
    }),

    openCreateProductModal() {
      this.$v.$reset();
      this.form = {
        id: '',
        name: '',
        description: '',
      };

      this.modals.currentModal = 'create';
      this.modals.isCreateProductModalVisible = true;
    },

    closeCreateProductModal() {
      this.modals.isCreateProductModalVisible = false;
    },

    async openEditModal(productId: string) {
      const product = await this.getProduct(productId);

      if (!product) {
        alert.error('Esse produto não existe mais').then(this.fetchProducts);
        return;
      }

      this.form = {
        id: product.id,
        name: product.name,
        description: product.description,
      };

      this.modals.currentModal = 'edit';
      this.modals.isCreateProductModalVisible = true;
    },

    async openDetailsModal(productId: string) {
      const product = await this.getProduct(productId);

      if (!product) {
        alert.error('Esse produto não existe mais').then(this.fetchProducts);
        return;
      }

      this.form = {
        id: product.id,
        name: product.name,
        description: product.description,
      };

      this.modals.isProductDetailsModalVisible = true;
    },

    async getProduct(productId: string) {
      const { data } = await productsRequests.getById(productId);

      return data.data;
    },

    submitForm(event: Event) {
      event.preventDefault();

      const submits = {
        create: this.submitCreateForm,
        edit: this.submitEditForm,
      } as const;

      submits[this.modals.currentModal]();

      return;
    },

    async submitCreateForm() {
      const response: ActionResponse = await this.addProduct(this.form);

      if (response.status === 201) {
        this.fetchProducts();
        this.modals.isCreateProductModalVisible = false;
        showToast({ icon: 'success', text: response.message });
      } else {
        alert.error(response.message);
      }
    },

    async submitEditForm() {
      const response: ActionResponse = await this.updateProduct(this.form);

      if (response.status === 200) {
        this.fetchProducts();
        this.modals.isCreateProductModalVisible = false;
        showToast({ icon: 'success', text: response.message });
      } else {
        alert.error(response.message);
      }
    },

    async confirmProductDelete(productId: string) {
      const { answer } = await alert.question({
        text: 'Essa operação não poderá ser desfeita',
      });

      if (answer === 'yes') {
        const deleteResponse: ActionResponse = await this.deleteProduct(
          productId
        );

        const isSuccessResponse = deleteResponse.status === 200;

        if (isSuccessResponse) {
          this.fetchProducts();
        }

        showToast({
          icon: isSuccessResponse ? 'success' : 'error',
          text: isSuccessResponse
            ? 'Produto excluído com sucesso'
            : deleteResponse.message,
        });
      } else {
        showToast({ icon: 'info', text: 'O produto não foi excluído' });
      }
    },
  },

  validations: {
    form: {
      name: { required, minLength: minLength(4) },
      description: { maxLength: maxLength(150) },
    },
  },
});
</script>

<style>
label {
  font-weight: 600;
}
</style>
